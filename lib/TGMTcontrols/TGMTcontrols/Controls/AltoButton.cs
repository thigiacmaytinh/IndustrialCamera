...
namespace TGMTcontrols
{
    ...
    public class AltoButton : Control, IButtonControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Rectangle textBounding = this.ClientRectangle;
            ...
            if (Icon != null)
            {
                int padding = (this.Height - this.ImageSize.Height) / 2;
                int iconWidth = Math.Min(this.Height - padding * 2, this.Width / 3); // Icon size based on button size
                int iconHeight = Math.Min(this.Height - padding * 2, this.Width / 3); // Icon size based on button size
                if (m_imageSize.Width > 0) iconWidth = m_imageSize.Width;
                if (m_imageSize.Height > 0) iconHeight = m_imageSize.Height;


                int totalWidth = iconWidth + padding + (int)g.MeasureString(this.Text, this.Font).Width;
                int iconX = m_imageLocation.X > 0 ? m_imageLocation.X : padding;
                int iconY = m_imageLocation.Y > 0 ? m_imageLocation.Y : (this.Height - iconHeight) / 2;


                g.DrawImage(Icon, new Rectangle(iconX, iconY, iconWidth, iconHeight));

                textBounding.X += iconWidth;
                textBounding.Width -= iconWidth;
            }
            ...
            {
                e.Graphics.DrawString(Text, Font, brush, textBounding, sf);
            }
